...
Radix UI Dialog ベースのボトムシートとして表示される。モバイルでは画面下部からスライドアップし（最大高さ90svh、上部に装飾的ドラッグハンドル表示）、デスクトップ（sm以上）では従来通り中央モーダル（max-w-lg）として表示。ESCキー、オーバーレイクリック、×ボタンで閉じることができ、フォーカストラップによるアクセシビリティに対応。`BottomSheetTitle` / `BottomSheetDescription`（スクリーンリーダー向け）を含む。おにぎりの有無により表示内容が変わる。
...
- セルパディング: `p-1 sm:p-2`
- ボトムシート: モバイルでは画面下部からスライドアップ表示（max-h-[90svh]）、デスクトップでは中央モーダル表示（max-w-lg）
