...
- 「カレンダー」「検索」の2つのタブをセグメントコントロール形式で表示（`bg-muted`の外枠にオレンジ色のスライドインジケーター）
- アクティブタブ切替時にインジケーターがスライドアニメーション（`transition-transform duration-300`）で移動
- ARIA属性（`role="tablist"` / `role="tab"` / `aria-selected`）によるアクセシビリティ対応
...
- **日付セル**: 7列のグリッドで各日を表示（モバイル: 高さ96px / デスクトップ: 高さ144px）
  ...
  - おにぎり保存後、該当セルにパルスアニメーション（scale + オレンジ色box-shadow、0.6秒）を表示
- **空状態ガイダンス**: 当月におにぎりが1件も登録されていない場合、カレンダー下部に「🍙 日付をタップしておにぎりを記録しましょう」のガイダンスメッセージを表示
...
- 検索結果が0件の場合: `SearchX`アイコン（Lucide）付きの空状態表示（「おにぎりが見つかりませんでした」「検索条件を変更して再度お試しください」）

### 3.4 おにぎり詳細/登録ダイアログ

Radix UI Dialog ベースのオーバーレイダイアログとして表示される。モバイルではフルスクリーンオーバーレイ（`fixed inset-0`、フェードイン/アウト）、デスクトップ（sm以上）では中央モーダル（max-w-lg, max-h-[85vh]）として表示。ESCキー、オーバーレイクリック、×ボタンで閉じることができ、フォーカストラップによるアクセシビリティに対応。`DialogTitle` / `DialogDescription`（スクリーンリーダー向け）を含む。おにぎりの有無により表示内容が変わる。

**表示モード（おにぎりが登録済みの場合）:**

- ヘッダーに「{日付}のおにぎり」とおにぎり名・店舗名を表示
- ヒーロー画像: おにぎり画像がある場合、ダイアログ上部に高さ192pxで大きく表示。グラデーションオーバーレイでおにぎり名を白文字で重ねて表示
- 情報カード: 店舗名・価格・評価を3カラムのカード（`bg-muted/50 rounded-lg`）にまとめて表示
- 写真比較: おにぎり写真と食べた時の写真を横並び（`grid-cols-2`、各h-32）で表示
- メモを全文表示（line-clamp なし）
- フッターに「削除」「閉じる」「編集」ボタン。削除ボタンはゴミ箱アイコン付きで左寄せ（`mr-auto`）、赤文字の `variant="outline"` スタイル。クリックするとフッターがインライン確認UIに切り替わり、「本当に削除しますか？」テキストと「キャンセル」「削除する」ボタンを表示。削除処理中はボタンが無効化され「削除中...」と表示

**編集モード（新規登録、または既存おにぎりの編集）:**

- ヘッダーに「{日付}のおにぎりを登録」または「{日付}のおにぎりを編集」と表示
- フォームは「Xからインポート」「基本情報」「写真」「メモ」の4セクションに分割。各セクションはヘッダーと区切り線で視覚的にグルーピング

**Xからインポートセクション:**

| フィールド | 入力タイプ | 説明 |
|-----------|----------|------|
| ポストURL | テキスト（Linkアイコン付き） | X（Twitter）のポストURL。貼り付け時に自動取得を開始。「取得」ボタンで手動実行も可能 |

- 有効なXポストURL（`x.com` / `twitter.com`）を検出するとFxTwitter API経由でポストデータを取得
- テキストの1行目を店舗名（絵文字除去、略称→正式名称変換: セブン→セブンイレブン、ファミマ→ファミリーマート）、2行目を商品名+価格として解析しフォームに自動入力
- ポスト添付画像（常に2枚）をSupabase Storageにダウンロード・リサイズ保存し、`imageUrl`（1枚目）・`eatImageUrl`（2枚目）に設定
- ポスト全文をメモ欄に設定
- 取得中はボタンにスピナー表示。成功時は「ポストからデータを取得しました」、エラー時は赤文字でエラーメッセージを表示
...
| 評価 | 星クリック (1〜5)、大きめサイズ（`text-3xl`） | いいえ | デフォルト3。星をクリックして設定。タップ時にスケールアニメーション |

**写真セクション:**

| フィールド | 入力タイプ | 必須 | 説明 |
|-----------|----------|------|------|
| おにぎりの写真 | カード型プレースホルダー（「＋写真を追加」）。タップでファイル選択。「URLを直接入力する」トグルでURL入力欄を展開可能 | いいえ | 画像アップロードまたは直接URL入力。画像がある場合カード内にプレビュー表示 |
| 食べた時の写真 | カード型プレースホルダー（「＋写真を追加」）。タップでファイル選択。「URLを直接入力する」トグルでURL入力欄を展開可能 | いいえ | 画像アップロードまたは直接URL入力。2枚を横並び（`grid-cols-2`）で表示 |
...
- インラインバリデーション: 必須フィールドからフォーカスが離れた時に空であれば赤枠とエラーメッセージを表示。保存ボタン押下時に全必須フィールドのエラーを一括表示
...
| メソッド | 説明 | パラメータ |
|---------|------|----------|
| POST | XポストURLからテキスト・画像を取得しテキスト解析結果を返す | ボディ: url（必須） |

- FxTwitter API（`api.fxtwitter.com`）を使用してポストデータを取得（認証不要）
- レスポンス: `{ text: string, images: string[], parsed: { name, storeName, price } }`
- テキスト解析（`tweet-parser.ts`）: 1行目→店舗名（絵文字除去、略称→正式名称変換）、2行目→商品名+価格（なければ0円）

### 6.3 サービス層 (`OnigiriService`)
...
| `delete(id)` | おにぎりを削除（DBレコードのみ） |
| `deleteWithImages(onigiri)` | おにぎりを画像ごと削除（Supabaseストレージの画像をベストエフォートで削除した後、DBレコードを削除） |
...
6. 保存後、トースト通知「おにぎりを記録しました 🍙」を表示し、ダイアログを閉じ、カレンダーの表示を更新（該当セルにパルスアニメーション）
7. エラー時はトースト通知「おにぎりの保存に失敗しました。もう一度お試しください。」を表示
...
1. 表示モードのフッターで「削除」ボタンをクリック
2. フッターがインライン確認UIに切り替わり、「本当に削除しますか？」と「キャンセル」「削除する」ボタンを表示
3. 「削除する」をクリックすると `OnigiriService.deleteWithImages()` を呼び出し
4. Supabaseストレージから関連画像（`imageUrl`, `eatImageUrl`）をベストエフォートで削除（画像削除に失敗してもDB削除は続行）
5. データベースからおにぎりレコードを削除
6. ローカルステートから該当データを除去し、カレンダー表示を更新
7. 保存後、トースト通知「おにぎりを削除しました」を表示し、ダイアログを閉じる
8. エラー時はトースト通知「おにぎりの削除に失敗しました。もう一度お試しください。」を表示
...
1. 登録ダイアログの「Xからインポート」セクションにXポストのURLを貼り付け
2. 有効なURL検出時に自動で `POST /api/fetch-tweet` を呼び出し（手動「取得」ボタンも使用可能）
3. サーバー側でFxTwitter API（`https://api.fxtwitter.com/i/status/{tweetId}`）からポストデータを取得
4. テキスト解析: 1行目→店舗名（絵文字除去、略称→正式名称変換）、2行目→商品名+価格（`\d+円` パターン、なければ0円）
5. 解析結果をフォームの空フィールドに自動入力（既に値があるフィールドは上書きしない）
6. ポスト全文をメモ欄に設定
7. 添付画像2枚をクライアント側でダウンロードし、`browser-image-resizer`でリサイズ（最大500px、品質0.85）後、Supabase Storageにアップロード
8. 1枚目→`imageUrl`、2枚目→`eatImageUrl` に設定
9. エラー時はインラインでエラーメッセージを表示（フォームの手動入力は引き続き可能）
...
- セルパディング: `p-1 sm:p-2`
- ダイアログ: モバイルではフルスクリーンオーバーレイ（fixed inset-0）、デスクトップでは中央モーダル表示（max-w-lg, max-h-[85vh]）
...
| lastSavedDate | string \| null | 保存アニメーション対象のセル日付文字列 |
