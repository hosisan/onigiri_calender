...
Radix UI Dialog ベースのボトムシートとして表示される。モバイルでは画面下部からスライドアップし（最大高さ100svh、上部に装飾的ドラッグハンドル表示）、デスクトップ（sm以上）では従来通り中央モーダル（max-w-lg）として表示。ESCキー、オーバーレイクリック、×ボタンで閉じることができ、フォーカストラップによるアクセシビリティに対応。`BottomSheetTitle` / `BottomSheetDescription`（スクリーンリーダー向け）を含む。おにぎりの有無により表示内容が変わる。

**表示モード（おにぎりが登録済みの場合）:**

- ヘッダーに「{日付}のおにぎり」とおにぎり名・店舗名を表示
- ヒーロー画像: おにぎり画像がある場合、ボトムシート上部に高さ192pxで大きく表示。グラデーションオーバーレイでおにぎり名を白文字で重ねて表示
...
- セルパディング: `p-1 sm:p-2`
- ボトムシート: モバイルではフルスクリーン表示（h-[100svh]）、デスクトップでは中央モーダル表示（sm:h-auto, max-w-lg, max-h-[85vh]）
