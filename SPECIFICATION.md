...
- **月ナビゲーション**: 左右の矢印ボタン（タップ領域 `h-12 w-12`）で前月・次月に移動。現在月以外を表示中は「今日」ボタンで今月に戻れる
- **スワイプナビゲーション**: 左右スワイプで前月・次月に移動（閾値50px、水平移動が垂直移動より大きい場合のみ発火）
- **曜日ヘッダー**: 日〜土の7列。日曜は赤色、土曜は青色で表示。モバイルでは小さいフォントサイズ（`text-xs sm:text-base`）
- **日付セル**: 7列のグリッドで各日を表示（モバイル: 高さ96px / デスクトップ: 高さ144px）
  - 当月の日付は `bg-card` 背景、前月・次月の日付は `bg-muted/50` 背景（ダークモード自動対応）
  - おにぎりが登録されている日にはサムネイル画像をセル背景として大きく表示（モバイル含む）。グラデーションオーバーレイで日付テキストの視認性を確保。画像がない場合はオレンジ色のドットインジケーターを表示
  - 登録されたおにぎりの名前を最大1件、セル下部に表示
...
- カレンダーセル: モバイル96px / デスクトップ144px（`h-24 sm:h-36`）
- おにぎりサムネイル画像: モバイルでも背景画像として表示。グラデーションオーバーレイで日付テキストの視認性を確保
- スワイプ操作: タッチイベントによる月間ナビゲーション対応（閾値50px）
