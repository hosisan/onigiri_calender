      ...
      <div className="grid grid-cols-7 gap-1">
        {calendarDays.map((date, index) => {
          ...
          return (
            <button
              key={index}
              className={`relative h-20 sm:h-32 p-1 sm:p-2 border border-border rounded-lg transition-colors flex flex-col items-start justify-start ${
                isCurrentMonthDay
              ...
              {hasOnigiri && (
                <div className="mt-1 w-full overflow-hidden">
                  {onigiriData[dateString].slice(0, 1).map((onigiri) => (
                    <div
                      key={onigiri.id}
                      className="text-xs bg-orange-50 dark:bg-orange-950 p-1 rounded text-foreground"
                      title={onigiri.name}
                    >
                      <div className="truncate mb-0.5 text-[10px] sm:text-xs">{onigiri.name}</div>
                      {onigiri.imageUrl && (
                        <div className="w-full h-16 rounded-sm overflow-hidden hidden sm:block">
                          <Image
